  ...
  if search == 'flat_q':
    ...
    space = [
        ...
        # },
        {
            ...
            "seed": tune.grid_search([5]),
            "group": tune.grid_search(['usfa_dyna-41-no-share']),
            "eval_task_support": tune.grid_search(["train"]),
            "dyna_coeff": tune.grid_search([1e-2]),
            # "gpi_eval_model": tune.grid_search([True]),
            # "mask_zero_features": tune.grid_search([True]),
            "policy_rep": tune.grid_search(["task_id"]),
            'discount': tune.grid_search([.6]),
            'sf_layers': tune.grid_search([[128]]),
            'policy_layers': tune.grid_search([[]]),
            'head': tune.grid_search(['monolithic', 'independent']),
            'env.terminate_failure': tune.grid_search([True, False]),
        },
        # {
        #     # "num_steps": tune.grid_search([5e6]),
        #     # "env.basic_only": tune.grid_search([1]),
